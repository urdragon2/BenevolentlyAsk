...
class PrefsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        mypassword = prefs.string(forKey: "password")!
        myemail = prefs.string(forKey: "email")!
        getData()
    ...
    func getData()
    {
        var request = URLRequest(url: URL(string: SERVER + "/getdata.php")!)
                ...
                else
                {
                    do {
                        ...
                        if let convertedJsonIntoDict = try JSONSerialization.jsonObject(with: data, options: []) as? NSArray {
                            let usernameget = (convertedJsonIntoDict[0] as! NSDictionary)["username"] as? String
                            let emailget = (convertedJsonIntoDict[0] as! NSDictionary)["email"] as? String
                            DispatchQueue.main.async() { () -> Void in
                                self.usernametext.text = usernameget
                                self.emailtext.text = emailget
                                self.passwordtext.text = self.mypassword
                            }
                        }
                        else{
                            //print("here")
    ...
    */
    ...
    func myvalidate()
    {
        ...
        {
            var request = URLRequest(url: URL(string: SERVER + "/updateuser.php")!)
            request.httpMethod = "POST"
            var postString = "auth_user="+ADMINILOGIN
            postString += "&auth_password="+ADMINIPASSWORD
            postString += "&id="+id
                ...
                if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode != 200 {           // check for http errors
                    ...
                    else
                    {
                        do {
                            ...
                            if let convertedJsonIntoDict = try JSONSerialization.jsonObject(with: data, options: []) as? NSArray {
                                OperationQueue.main.addOperation {
                                    self.dialogOKCancel("Settings", newmessage: "Settings have been saved.")
                                }
                            }
                            else{
                                //print("here")
